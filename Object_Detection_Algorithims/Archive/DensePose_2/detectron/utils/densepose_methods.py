...
class DensePoseMethods:
  def __init__(self):
    #
    ALP_UV = loadmat(os.path.join(os.path.dirname(
        __file__), '../../DensePoseData/UV_data/UV_Processed.mat'))
    ...
    self.Index_Symmetry_List = [1, 2, 4, 3, 6, 5, 8, 7, 10, 9,
                                12, 11, 14, 13, 16, 15, 18, 17, 20, 19, 22, 21, 24, 23]
    UV_symmetry_filename = os.path.join(os.path.dirname(
        __file__), '../../DensePoseData/UV_data/UV_symmetry_transforms.mat')
  ...
  def barycentric_coordinates_fast(self, P0, P1, P2, P):
    ...
    # vector-wise dot product. Result shape is (n,)
    A = np.einsum('nd,nd->n', vCrossW, vCrossU)
    #
    uCrossW = np.cross(u, w)
    uCrossV = - vCrossU
    ...
    #
    # result shape is  (n,)
    sq_denoms = np.einsum('nd,nd->n', uCrossV, uCrossV)
  ...
  def IUV2FBC_fast(self, I_point, U_point, V_point):
    ...
    #
    P_0 = np.vstack((self.U_norm[FacesNow][:, 0], self.V_norm[FacesNow][:, 0], np.zeros(
        self.U_norm[FacesNow][:, 0].shape))).transpose()
    P_1 = np.vstack((self.U_norm[FacesNow][:, 1], self.V_norm[FacesNow][:, 1], np.zeros(
        self.U_norm[FacesNow][:, 1].shape))).transpose()
    P_2 = np.vstack((self.U_norm[FacesNow][:, 2], self.V_norm[FacesNow][:, 2], np.zeros(
        self.U_norm[FacesNow][:, 2].shape))).transpose()
    #
